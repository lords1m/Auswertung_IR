...
init_repo_paths();
    ...
    % Positionsfilter (für Pegel über Entfernung)
    hLblPosFilter = uicontrol(pnlControl, 'Style', 'text', 'Position', [10 65 200 15], ...
                              'String', 'Positionen (z.B. 1,4,7):', ...
                              'HorizontalAlignment', 'left', 'Visible', 'off');
    hEditPosFilter = uicontrol(pnlControl, 'Style', 'edit', 'Position', [10 45 200 20], ...
                               'String', '', 'Visible', 'off', 'Callback', @updatePlot);

    % RT60-Durchschnitt ein/aus
    hShowAvgV1 = uicontrol(pnlControl, 'Style', 'checkbox', 'Position', [10 45 200 20], ...
                           'String', 'Durchschnitt anzeigen', 'Value', 0, ...
                           'Callback', @onToggleAvgV1, 'Visible', 'off');
    ...
    function onToggleAvgV1(~, ~)
        % Eigener Callback, damit die Checkbox in RT60 immer unmittelbar
        % eine Neudarstellung ausloest.
        updatePlot();
    end
    ...
    function updatePlot(~, ~)
        ...
        if plotType == 7 % Heatmap
            ...
            if plotType == 5
                set(hLblPosFilter, 'Visible', 'on');
                set(hEditPosFilter, 'Visible', 'on');
            else
                set(hLblPosFilter, 'Visible', 'off');
                set(hEditPosFilter, 'Visible', 'off');
            end

            if plotType == 8
                set(hShowAvgV1, 'Visible', 'on');
            else
                set(hShowAvgV1, 'Visible', 'off');
            end
        ...
        switch plotType
            ...
            case 5 % PEGEL ÜBER ENTFERNUNG
                [selected_positions, filter_ok] = parse_position_filter(get(hEditPosFilter, 'String'));
                if ~filter_ok
                    title('Ungueltiger Positionsfilter');
                    text(0.5, 0.5, 'Gueltiges Format: 1,4,7 oder leer fuer alle Positionen', ...
                         'Units', 'normalized', 'HorizontalAlignment', 'center');
                    return;
                end

                [dist1, lev1, ~] = get_variant_levels(R1.meta.variante, hSource1.Value, useEnergyMode, selected_positions);
                ...
                if isCompare
                    [dist2, lev2, ~] = get_variant_levels(R2.meta.variante, hSource2.Value, useEnergyMode, selected_positions);
                ...
                if isempty(selected_positions)
                    infoText = 'Punkte zeigen Messpositionen 1-15';
                else
                    infoText = sprintf('Punkte zeigen gefilterte Positionen: %s', strtrim(get(hEditPosFilter, 'String')));
                end
                text(0.02, 0.05, infoText, 'Units', 'normalized', 'FontSize', 8, 'Color', [0.5 0.5 0.5]);
            ...
            case 8 % NACHHALLZEIT (RT60) ÜBER FREQUENZ
                ...
                if isCompare
                    ...
                    if numel(idx1) > 1
                        colorMap = lines(numel(idx1));
                        sameVariante = true;
                        baseVariante = R1.meta.variante;

                        for k = 1:numel(idx1)
                            name_k = list1{idx1(k)};
                            Rk = loadData(name_k, hSource1.Value);

                            if isfield(Rk.freq, 't30') && ~isempty(Rk.freq.t30)
                                t30_k = Rk.freq.t30;
                            else
                                [t30_k, ~] = calc_rt60_spectrum(Rk.time.ir, Rk.meta.fs);
                            end

                            t30_k_plot = [t30_k, t30_k(end)];
                            stairs(x_plot, t30_k_plot, 'LineWidth', 1.5, ...
                                   'Color', colorMap(k,:), ...
                                   'DisplayName', cleanName(name_k)); hold on;

                            if ~strcmp(Rk.meta.variante, baseVariante)
                                sameVariante = false;
                            end
                        end

                        if sameVariante && hShowAvgV1.Value
                            [avg_t30_multi, ~] = get_avg_rt60(baseVariante, hSource1.Value);
                            avg_t30_multi_plot = [avg_t30_multi, avg_t30_multi(end)];
                            stairs(x_plot, avg_t30_multi_plot, 'k--', 'LineWidth', 1.2, ...
                                   'DisplayName', ['Ø ' strrep(baseVariante, '_', ' ')]);
                        end

                        legend show;
                        title(sprintf('Nachhallzeit RT60: Mehrfachauswahl (%d Positionen)', numel(idx1)));
                    else
    ...
    function [dist, levels, positions] = get_variant_levels(variante, sourceType, energyMode, selected_positions)
        geo = get_geometry();
        ...
        for i = 1:length(files)
            try
                ...
                if ~isempty(selected_positions) && ~ismember(posNum, selected_positions)
                    continue;
                end
    ...
    function [positions, ok] = parse_position_filter(pos_text)
        txt = strtrim(pos_text);
        if isempty(txt)
            positions = [];
            ok = true;
            return;
        ...
        tokens = regexp(txt, '[,;\\s]+', 'split');
        tokens = tokens(~cellfun(@isempty, tokens));
        vals = [];

        for i = 1:numel(tokens)
            v = str2double(tokens{i});
            if isnan(v) || v < 1 || v > 15 || round(v) ~= v
                positions = [];
                ok = false;
                return;
            end
