...
% Analysiert den Pegelabfall im Vergleich zur 1/r Kurve.
...
% 1/3 Leistung in dB: 10*log10(1/3) ≈ -4.77 dB relativ
y_third_level = max(all_levels) + 10*log10(1/3);
...
saveas(fig1, fullfile(outputDir, save_name));
savefig(fig1, fullfile(outputDir, strrep(save_name, '.png', '.fig')));
fprintf('✓ Plot gespeichert: %s (.png + .fig)\n', fullfile(outputDir, save_name));
...
saveas(fig2, fullfile(outputDir, save_name_scatter));
savefig(fig2, fullfile(outputDir, strrep(save_name_scatter, '.png', '.fig')));
fprintf('✓ Plot gespeichert: %s (.png + .fig)\n', fullfile(outputDir, save_name_scatter));
...
for v_idx = 1:numel(results)
    ...
    saveas(fig2b, fullfile(outputDir, save_name_2d_path));
    savefig(fig2b, fullfile(outputDir, strrep(save_name_2d_path, '.png', '.fig')));
    fprintf('✓ Plot gespeichert: %s (.png + .fig)\n', fullfile(outputDir, save_name_2d_path));
end

%% Plot 2c: Vergleich mit Differenz
if numel(results) == 2
    ...
    for pos = common_positions
        ...
        if ~isempty(idx1) && ~isempty(idx2)
            ...
            if y1 > y2
                line_color = [1 0.7 0.7];
            else
                line_color = [0.7 0.7 1];
            end
    ...
    saveas(fig2c, fullfile(outputDir, save_name_compare));
    savefig(fig2c, fullfile(outputDir, strrep(save_name_compare, '.png', '.fig')));
    fprintf('✓ Plot gespeichert: %s (.png + .fig)\n', fullfile(outputDir, save_name_compare));
else
...
if numel(results) == 2
    ...
    for pos = common_positions
        ...
        if ~isempty(idx1) && ~isempty(idx2)
            ...
            if ~isempty(pos_info_idx)
                ...
                if z1 > z2
                    line_color = [1 0.5 0.5];
                else
                    line_color = [0.5 0.5 1];
                end
    ...
    saveas(fig2d, fullfile(outputDir, save_name_3d_comp));
    savefig(fig2d, fullfile(outputDir, strrep(save_name_3d_comp, '.png', '.fig')));
    fprintf('✓ Plot gespeichert: %s (.png + .fig)\n', fullfile(outputDir, save_name_3d_comp));
else
    fprintf('ℹ 3D Vergleichsplot übersprungen (benötigt genau 2 Varianten, %d vorhanden)\n', numel(results));
end

%% Plot 3: 3D Einzelansicht
...
for v_idx = 1:numel(results)
    ...
    saveas(fig3, fullfile(outputDir, save_name_3d));
    savefig(fig3, fullfile(outputDir, strrep(save_name_3d, '.png', '.fig')));
    fprintf('✓ Plot gespeichert: %s (.png + .fig)\n', fullfile(outputDir, save_name_3d));
end
...
for v_idx = 1:numel(results)
    ...
    % Abweichung
    if strcmp(darstellung_modus, 'leq')
        L_ideal_at_meas = -20*log10(results(v_idx).distances / r_ref);
    else
