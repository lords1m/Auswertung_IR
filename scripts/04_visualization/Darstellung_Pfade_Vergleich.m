...
pfade = {
    ...
    'Pfad 2: M1-M2-M3-M4',    [1, 2, 3, 4],    [0, 2.0], [-5, 10];
...
pfade_auswahl = [1,2,3];  % <-- HIER ANPASSEN
...
% Projektverzeichnis ermitteln (zwei Ebenen über scripts/04_visualization)
projectDir = fileparts(fileparts(scriptDir));
procDir = fullfile(projectDir, 'processed');
...
%% Summenpegel pro Pfad und Variante ausgeben

fprintf('\n');
fprintf('================================================================================\n');
fprintf('SUMMENPEGEL PRO PFAD UND VARIANTE\n');
fprintf('================================================================================\n');
...
for pfad_idx = pfade_auswahl
    ...
    fprintf('\n%s\n', pfad_name);
    fprintf('--------------------------------------------------------------------------------\n');
    fprintf('%-15s | %-12s | %s\n', 'Variante', 'Summenpegel', 'Positionen');
    fprintf('--------------------------------------------------------------------------------\n');
    ...
    for v_idx = 1:numel(results)
        ...
        % Energien für diesen Pfad sammeln
        pfad_energien = [];
        pfad_pos_str = '';
        ...
        for pos = pfad_positionen
            pos_result_idx = find(results(v_idx).positions == pos, 1);
            if ~isempty(pos_result_idx)
                % Energie (nicht dB) für korrekte Summierung
                pfad_energien(end+1) = results(v_idx).leq_values(pos_result_idx);
                if isempty(pfad_pos_str)
                    pfad_pos_str = sprintf('M%d', pos);
                else
                    pfad_pos_str = sprintf('%s, M%d', pfad_pos_str, pos);
                end
            end
        end

        if ~isempty(pfad_energien)
            % Summenpegel: 10 * log10(sum(Energien))
            summenpegel = 10 * log10(sum(pfad_energien));
            variante_name = strrep(results(v_idx).variante, '_', ' ');
            fprintf('%-15s | %8.2f dBFS | %s\n', variante_name, summenpegel, pfad_pos_str);
        end
    end
end
